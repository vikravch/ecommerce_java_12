...
const SortItem: React.FC = () => {
	...
	const [sortItem, setSortItem] = React.useState(list[0].value)
	...
	// }, [])
	//
	// let menuRef = React.useRef<any>(null)
	
	return (
		<div
			// ref={menuRef}
			onClick={handleTogglePopup}
			// onClick={(e) => setSortItem(e.currentTarget)}
				...
				{list.map(({value}: ListItem, id) => (
					...
					>
						<div
							...
							onClick={() => handleChangeSortItem(value)}
						>{value}
						</div>
						<hr/>
					</div>
